...
void drawGrid(float cellSize, int cols, int rows, int minSize, int maxSize, int posn) {
  ...
  for (int i = 0; i < cols; i++) {
    ...
    for (int j = 0; j < rows; j++) {
      ...
      if (drawingType == 0) {
        ...
        fill(255);
        ellipse(x, y + posn + random(6), random(minSize, maxSize), random(minSize, maxSize));
        fill(0, 0, 255, random(0, 100));
        ellipse(x, y + posn + random(3), 25, 25);
      } else if (drawingType == 1) {
        // If 1, draw semi-transp blue circ
        rotate(0);
        fill(0, 0, 255, random(255));
        ellipse(x, y + posn + random(3), random(minSize, maxSize), random(minSize, maxSize));
      }
    }
  }
}
